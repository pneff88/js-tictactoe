...
//game board module;
let gambeBoard = (() => {
    let squares = document.querySelectorAll(".grid-item"); //this is a nodelist of the square divs, ids 1-9
...
//sqaures event listeners module
let eventListeners = (() => {
...
let gameFlow = (() => {
    ...
    let convertArray = (x) => { //what are we doing here??
    ...
    let checkMoves = (x) => {
        convertArray(x); //defined in 76ff
        for (i=1; i<10; i++) {
            if (
                (i===1 || i === 4 || i=== 7) && //if move is in first column
                x.includes(i) &&
                x.includes(i+1) &&
                x.includes(i+2) //and includes rest of row, win horizontally 
            ) {
                win();
                break;
            } else if (
                ...
                x.includes(i) && 
                x.includes(i+2) && 
                x.includes(i+4) //win diagonaly
            ) {
                win();
                break;
            } else if (x.includes(i) && x.includes(i+3) && x.includes(i+6)) { //win vertically
                win();
                break;
            } else if (x.includes(i) && x.includes(i+4) && x.includes(i+8)) { //win diagonally the other way
                win();
                break;
            }
        }
    ...
    let addPoint = () => { //not how the logic is neatly nested in this function and here accounts for whose tur it is
        if (gameFlow.moveCount % 2 === 0) {
            players[1].winCount++;
            gameFlow.moveCount = 2;  //what's going on here?
        } else {
            players[0].winCount++;
